...
async def get_notifications(request: Request, limit, offset):
    ...
    async with async_session() as session:
        select_stmt = (
            select(Notifications).where(Notifications.user_id == token_data["user_id"]).limit(limit).offset(offset)
        )
        ...
        return responses.JSONResponse(
            {"message": f"Got {notifications_num} notifications.", "notifications": payload},
...
async def create_notification(request: Request):
    ...
    return responses.JSONResponse(
        {"message": "Notification was created.", "notification_id": notification_id}, status_code=status.HTTP_200_OK
    )
...
async def update_notification(request: Request):
    ...
    update_values = {k: v for k, v in payload.items() if k in ("title", "body", "notify_time")}
    # todo validation columns

    async with engine.connect() as conn:
        select_stmt = select(Notifications.user_id).where(Notifications.id == notification_id)
        ...
        if notification_user_id == user_id:
            ...
            async with engine.connect() as conn:
                update_stmt = (
                    ...
                    .where(Notifications.id == user_id)
            ...
            return responses.JSONResponse(
                {"message": "Notification does not belong to the user."}, status_code=status.HTTP_403_FORBIDDEN
            )

    return responses.JSONResponse(
        {"message": "Notification was updated.", "notification_id": notification_id}, status_code=status.HTTP_200_OK
    )
...
async def delete_notifications(request: Request):
    ...
    async with async_session() as session:
        result = await session.execute(select(Notifications).where(Notifications.id.in_(request_notification_ids)))
        accepted_notification_ids = [
            n["Notifications"].id for n in result.all() if n["Notifications"].user_id == user_id
        ]
